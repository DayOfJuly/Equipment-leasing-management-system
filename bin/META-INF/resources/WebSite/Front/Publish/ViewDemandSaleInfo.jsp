...
<script type="text/javascript">
        var app = angular.module('viewDemanApp', ['ngResource', 'unifyModule','ngMessages', 'myPagination','Config','sysCodeConfigModule','sysCodeTranslateModule']);
        app.controller('viewDemanController', function ($scope, $timeout ,$interval, DemandRentSvc,personSvc, DemandSaleSvc, PicSvc,regionSvc,published,PicUrl,busDealInfo,SYS_CODE_CON,IssuSvc,sysCodeTranslateFactory) {

       	 $scope.sysCodeCon=SYS_CODE_CON;//把常量赋值给一个对象这样可以使用了
       	    
       	 $scope.ct=sysCodeTranslateFactory;//把翻译赋值给一个对象

 	    var viewinfo="newSale";
 		$scope.cookiesJudger=function(rec){/* 判断cookies是否存在，如已存在直接进入页面，如不存在进入登录页面 */
 			dataId = rec.dataId;
 			infoType = rec.dataType;
 			var isLogin = null;
 				if(isLogin==null){
 							window.location.href="../../Back/Publish/ViewDemandSaleInfo.jsp?id="+dataId+"&infoType="+infoType+"&flag="+viewinfo;
 						}
 			};
            ...
            $scope.queryviewData = function () {
                ...
                function qSucc(rec){
                	...
                	if(rec.issueInfo.infoTitle){
                		if(rec.issueInfo.infoTitle.length > 20){
                    		$scope.viewList.infoTitleA = rec.issueInfo.infoTitle.substring(0,30)+"...";
                    	}else{
                    		$scope.viewList.infoTitleA=rec.issueInfo.infoTitle;
                    	}
                	}
                    ...
                    if($scope.depName){
                    	if($scope.depName.length>10  ){
	                   	 $scope.depNameA = rec.issueInfo.depName.substring(0,10)+"...";
	                	}else{
	                		$scope.depNameA = rec.issueInfo.depName;
	                	}
                    }
                  	//设备的使用城市
                ...
                published.unifydo({Action:"DemandSale"},{parm: $scope.requestParms.id}, qSucc, qErr);
            };
            $scope.infoType = $scope.requestParms.infoType;
            //同类产品推荐
            $scope.titleList=[];
             $scope.querySimilar = function(){
            		 ...
            		 function qSuc(rec) {
	                	 ...
	                	 function qErr(){}
	                	 IssuSvc.post({Action:"DemandSale"},{dataType:4,equName:$scope.equName,pageNo:0,pageSize:10},qSucc,qErr);
	            	 }
	            	 function qEr(rec) {};
	            	published.unifydo({Action:"DemandSale"},{parm: $scope.requestParms.id},qSuc,qEr);
...
<div class="main">
   ...
   <span><a href="../Main/HomePage.jsp?">最新求购</a></span>
      &nbsp; <span>></span><span  title="{{viewList.infoTitle}}">&nbsp;{{viewList.infoTitleA}}</span> &nbsp;</div>
   
  <div class="bdi_left">
       <div class="">

         <div class="beft_top_rightr" style="margin-left:50px;">
           <div id="ViewInfo_Div1"  title="{{viewList.infoTitle}}" style="font-size:16px;color:#000 "><b>【求购】{{viewList.infoTitleA}}</b></div>
		   ...
		   <p >购买单价：{{ct.formatCurrency(viewList.price)}}元</p>
	       <p >数量：   <span class="col_chen txt14">{{viewList.quantity}}{{ct.codeTranslate(viewList.second,"UNIT_NAME")}}</span> </p> 
			...
			<p>意向出售人：<span class="col_chen txt14">{{dealCount}}<span ng-if="dealCount==''" class="col_chen txt14">0</span>人</span></p>
	       </div>
           <div class="clear"></div>
              <div class="hiofoe_k">
             ...
             <ul class="uiig qichucss">
               <span style="word-wrap: break-word; word-break: normals;" ><span id="rePayMsg" style="text-indent:2em"></span></span>
   ...
   <div class="zb_right"> 
    <div class="gao_fe liok ghiuj">
       ...
       <ul class="news_ppl ihhaleftr" ng-show="hidden==1" >
         <li><span >联系单位：</span><font title="{{viewList.enterpriseName}}">{{viewList.enterpriseNameA}}</font></li>
          ...
          <div ng-if="!addButton"> 
          		...
          		<li ng-show="reveal==2" title="点击“我想交易”按钮后，可以看到联系电话"><span>联系电话： </span><font style="width:150px">{{viewList.contactPhone1}}</font></li>
          
          		<li ng-show="reveal==1"><span>QQ： </span><font>{{viewList.qqNo}}</font></li>
          		<li ng-show="reveal==2" title="点击“我想交易”按钮后，可以看到QQ号码"><span>QQ： </span><font style="width:150px">{{viewList.qqNo1}}</font></li>
          </div>
            <div ng-if="addButton"> 
       ...
       <ul class="news_ppl ihhaleftr" ng-show="hidden==2">
          <li><span>联系单位： </span><font>请  <a class="col_chen" ng-click="cookiesJudger(viewList)">登录</a> 查看</font></li>
          <li><span>所在城市： </span><font>请  <a class="col_chen" ng-click="cookiesJudger(viewList)">登录</a> 查看</font></li>
          <li><span>详细地址： </span><font>请  <a class="col_chen" ng-click="cookiesJudger(viewList)">登录</a> 查看</font></li>
          <li><span>联 系  人： </span><font>请  <a class="col_chen" ng-click="cookiesJudger(viewList)">登录</a> 查看</font></li>
          <li><span>联系电话： </span><font>请  <a class="col_chen" ng-click="cookiesJudger(viewList)">登录</a> 查看</font></li>
          <li><span>QQ： </span><font>请  <a class="col_chen" ng-click="cookiesJudger(viewList)">登录</a> 查看</font></li>
        <div class=" clear"></div>
      </ul>
    ...
    <div class="gao_fe liok nedgwg mar_t15">
       ...
       <ul class="news_ppl news_pdds">
        <li  ng-repeat="r in titleList" title="{{r.infoTitle}}">
          	<a href="../Publish/ViewDemandSaleInfo.jsp?id={{r.dataId}}&infoType=4" >· {{r.infoTitleA}}</a>
        </li>
      </ul>
       <div class=" clear"></div>
    </div>
