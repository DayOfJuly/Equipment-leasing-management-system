app.controller('RentCheckInListController', function($scope,$timeout,published,rentSvc,entSvc,SaleSvc,regionSvc,equipment,busPublishHist) {
	
	
	//ie9下titile正确显示使用
	document.title="发布结果登记";
	...
	*/
	...
	{
            if(parm==""){           	
            	$scope.queryData.beginDate = datastr;
            }
	};
	...
	{
		 if(parm==""){           	
         	$scope.queryData.endDate = $scope.initDate;
         }
    ...
    $scope.testFun = function(){
		...
		$scope.changeListValueA($scope.busPublishList,'equNo');
