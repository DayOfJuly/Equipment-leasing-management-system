...
/**
 * party统一接口*/
app.service('personOutEmployeeSvc', function($resource,perOutEmployeeUrl) {
	var actions = {
			queryPersonList:{method:'POST',params:{}},//查询人员列表
			queryPerson:{method:'GET',params:{id:'@id'}},//查询人员列表有id
			addPersonDetail:{method:'PUT',params:{}},//添加员工信息没id
 	...
 	return $resource(perOutEmployeeUrl+"/:urlPath/:id/:state/:upLoadId", {}, actions);	
});
...
app.service('regionSvc', function($resource,regionUrl) {
	var actions = {
			...
			queryRegionByRegionId: {method:'GET', params:{id: "@id"}}//	根据regionId，查询对应的地区信息
  	};
...
//检验是否存在此员工
app.service('CheckEmployeeTrueOrFalseSvc',function($resource,CheckEmployeeTrueOrFalseUrl){
	var actions = {
			unifydo:{method:'GET', params: {urlPath:"@urlPath",parm:"@parm"}}
	};
	return $resource(CheckEmployeeTrueOrFalseUrl+"/:urlPath", {}, actions);	
});

//检验是否存在此员工
app.service('CheckEmployeeTrueOrFalseOutSvc',function($resource,CheckEmployeeTrueOrFalseOutUrl){
	var actions = {
			unifydo:{method:'GET', params: {urlPath:"@urlPath",parm:"@parm"}}
	};
	return $resource(CheckEmployeeTrueOrFalseOutUrl+"/:urlPath", {}, actions);	
});
...
//设备维护
app.service('busEquParameterSvc',function($resource,BusEquParameterUrl){
	var actions = {
			unifydo:{method:'GET', params: {urlPath:"@urlPath",parm:"@parm"}},
			putUnifydo:{method:'PUT', params: {urlPath:"@urlPath",parm:"@parm"}},
			delUnifydo:{method:'DELETE', params: {urlPath:"@urlPath",parm:"@parm"}},
	};
	return $resource(BusEquParameterUrl+"/:urlPath", {}, actions);	
});
//统计报表
app.service('statisticsSvc',function($resource,StatisticsUrl){
	var actions = {
			queryStatisticsList:{method:"POST",params:{urlPath:"@urlPath"}},
	};
	return $resource(StatisticsUrl+"/:urlPath",{},actions);
});
//报表导出
app.service('statisticsExportSvc',function($resource,StatisticsExportUrl){
	var actions = {
			Export:{method:"POST",params:{urlPath:"@urlPath"}},
	};
	return $resource(StatisticsExportUrl+"/:urlPath",{},actions);
});
