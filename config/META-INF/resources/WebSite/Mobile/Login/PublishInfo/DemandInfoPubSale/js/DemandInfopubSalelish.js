app.controller('DemandInfopubSalelishController', function ($scope,$timeout,$route,DemandRentSvc, regionSvc, category, published) {
	if($scope.equ_List){
    ...
    $scope.selectSubmit=function(obj,param,input){
		...
		$scope.selectListSale.equipmentName = $scope.equipmentName;
	...
	*/
	...
	$scope.selectListSale.brandS="";
	$scope.selectListSale.brandI="";
	$scope.selectListSale.modelS="";
	$scope.selectListSale.modelI="";
	$scope.selectListSale.standardS="";
	$scope.selectListSale.satndardI="";
	$scope.addModelSubimt=function(){
		...
		if($scope.brandInputShow){
			/*赋值品牌*/
			//$scope.queryEquBrandList.newBrandName=$scope.selectListSale.brandI;
			$scope.queryEquBrandListCopy.newBrandName=$scope.selectListSale.brandI;
		}
		if($scope.modelInputShow){
			/*赋值型号*/
			$scope.queryEquModelList.newModelName=$scope.selectListSale.modelI;
		}
		if($scope.standardInputShow){
			/*赋值规格*/
			$scope.queryEquStandardList.newStandardName=$scope.selectListSale.satndardI;
		}
		...
		if($scope.standardSelectShow){
			...
			//$scope.selectListSale.manufacturer=$scope.queryEquBrandList.newBrandName;
			$scope.selectListSale.manufacturer=$scope.queryEquBrandListCopy.newBrandName;
			$scope.selectListSale.modelNumber=$scope.queryEquModelList.newModelName;		
			$scope.selectListSale.specifications=$scope.queryEquStandardList.newStandardName;
			
		
			$scope.$emit('selectListSale',{saveParm:$scope.selectListSale});
