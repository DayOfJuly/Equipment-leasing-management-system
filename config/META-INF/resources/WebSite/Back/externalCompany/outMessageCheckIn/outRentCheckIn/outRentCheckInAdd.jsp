			  ...
			  <form name="emForm" action="" method="post" id="form" enctype="application/x-www-form-urlencoded">
							...
							<tbody>
								<tr ng-repeat="mess in messList"  style="text-align:center;" ng-click="checkLine(this);">
									<td>
										...
										<p class="copyP" style="margin-left:13px;margin-top:-15px;">{{$index+1+(paginationConf.currentPage-1)*paginationConf.pagesLength}}</p>
									</td>					
			                        <td><input type="text" class="form-control" maxlength="30" ng-model="mess.depName" title="{{mess.depName}}" style="margin-top:0px;margin-bottom:0px;height:17px;font-size:12px"></td><!-- 使用单位 -->
			                        <td><input type="text" class="form-control" maxlength="50" ng-model="mess.startEndDate" title="{{mess.startEndDate}}" style="margin-top:0px;margin-bottom:0px;height:17px;font-size:12px"></td><!-- 起止日期 -->
			                        <td><input type="text" class="form-control" maxlength="18" ng-model="mess.rent" title="{{mess.rent}}" style="margin-top:0px;margin-bottom:0px;height:17px;font-size:12px"
			                         ng-change="formatMoney('messList','rent',2,$index);"></td><!-- 租赁单价 -->
									<td>
										<select name="state" id="state{{$index}}" 
									        ng-model="mess.rentType" ng-mouseover="changeZIndex($index,messList.length);"    class="form-control select-hover" style="margin-top:0px;margin-bottom:0px;height:17px;font-size:12px" >
									...
									<td>
										...
										<!-- <div ng-messages="emForm.rentCount{{$index}}.$error" ng-show="showFlag == 'rentCount{{$index}}'">
											<p class="form-control-static" style="color: red;" ng-message="pattern">请您填写正确的数量</p>
									    </div> -->
									</td><!-- 租期数量 -->
									<td><input type="text" class="form-control" maxlength="17" ng-model="mess.amount" title="{{mess.amount}}" ng-change="formatMoney('messList','amount',2,$index);"style="margin-top:0px;margin-bottom:0px;height:17px;font-size:12px" ></td>
									<td><input type="text" class="form-control" maxlength="17" ng-model="mess.cost" title="{{mess.cost}}"  ng-change="formatMoney('messList','cost',2,$index);" style="margin-top:0px;margin-bottom:0px;height:17px;font-size:12px"></td>
									<td><input type="text" class="form-control" maxlength="17" ng-model="mess.deductCost" title="{{mess.deductCost}}"  ng-change="formatMoney('messList','deductCost',2,$index);" style="margin-top:0px;margin-bottom:0px;height:17px;font-size:12px"></td>
									<td><input type="text" class="form-control" maxlength="30" ng-model="mess.note" title="{{mess.note}}" style="margin-top:0px;margin-bottom:0px;height:17px;font-size:12px"></td>
								</tr>
								</tbody>
						</table>
						<h4>租赁费登记-承租方</h4>
						<table class="table table-striped table-hover" style="width: 98%;margin-left:15px;">
							...
							<tbody>
								<tr ng-repeat="users in userRentInfos"  style="text-align:center;" >
									...
									<p class="copyP" style="margin-top:0px;text-align:center;">{{$index+1+(paginationConf.currentPage-1)*paginationConf.pagesLength}}</p>									</td>					
				...
				<div class="modal-footer">	 
					<input style="float: right;margin-top:-10px" type="button" class="btn btn_Under btn-default" value="取消" data-dismiss="modal" ng-click="cancel()"/>
			   		...
			   		<input style="float: right;margin-top:-10px" type="button" class="btn btn_Under btn-primary" value="保存"  ng-click="saveMess(emForm);" ng-disabled="add" />
			   		<input style="float: right;margin-top:-10px" type="button" class="btn btn_Under btn-primary" value="新增"  ng-click="addRow();"/>	
			   </div> 
			 </form>
		   </div>
		<!-- /.modal-content -->
