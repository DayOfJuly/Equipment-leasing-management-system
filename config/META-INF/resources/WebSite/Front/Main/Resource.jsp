...
app.controller('searchController',function($scope,$sanitize,unifyTestSvc1,category,rentSvc,SaleSvc,DemandRentSvc,DemandSaleSvc,regionSvc,published,entSvc,equipment,searchUrl,SYS_CODE_CON,sysCodeTranslateFactory){
...
$scope.worklostblur=function(){
	...
	*所在城市   city
	*/
	$scope.valuClick = function(param){
		if(param == null || param ==""){
			$scope.abcInputClick();
			return;
		}
		$scope.abcInputValue=param;
		var valuejson={"city":param};
		var namejson={"city":param};
		$scope.displayCondition(namejson);
		$scope.abcInputClick();
		riqi1.style.display='none';
	};
...
/* 	$scope.brandSQ=function(){
	...
	*/
	...
	/* 
	<!--ng-click="changePrice(false);"  ng-change="changePrice(true,0,minPrice)" ng-change="changePrice(true,1,maxPrice)"  ng-change="changePrice(true,2)"-->
	$scope.changePrice = function(parm1,parm2,parm3){
		if(!parm1){
			$scope.deletcondition("devicePrice");
			$scope.minPrice = "";
			$scope.maxPrice = "";
		}else{
			var devicePrice = {};
			var priceFlag = true;
			for(var i = 0; i < $scope.arraynameArray.length;i++){
				if($scope.arraynameArray[i].type == 'devicePrice'){
					devicePrice = $scope.arraynameArray[i];
					priceFlag = false;
					if(parm2 == 0){
						devicePrice.minPrice = parm3;
					}else if(parm2 == 1){
						devicePrice.maxPrice = parm3;
					}else if(parm2 == 2){
						devicePrice.priceUnit = $scope.priceUnit;
					}
					devicePrice.name = (devicePrice.minPrice == null?" ":devicePrice.minPrice)+"--"+(devicePrice.maxPrice == null?" ":devicePrice.maxPrice)+($scope.priceUnit ==0?"元/月":$scope.priceUnit==1?"元/天":"元/小时");
					$scope.arraynameArray[i] = devicePrice;
					if((devicePrice.minPrice==null || devicePrice.minPrice == "")&&(devicePrice.maxPrice==null || devicePrice.maxPrice == "")){
						$scope.deletcondition("devicePrice");
					}
					break;
				}
			...
			if(priceFlag){
				if(($scope.minPrice==null || $scope.minPrice == "")&&($scope.maxPrice==null || $scope.maxPrice == "")){
					$scope.deletcondition("devicePrice");
				}else{
					if(parm2 == 0){
						devicePrice.minPrice = $scope.minPrice;
					}else if(parm2 == 1){
						devicePrice.maxPrice = $scope.maxPrice;
					}else if(parm2 == 2){
						devicePrice.priceUnit = $scope.priceUnit;
					}
					devicePrice.type='devicePrice';
					devicePrice.name = (devicePrice.minPrice == null?" ":devicePrice.minPrice)+"--"+(devicePrice.maxPrice==null?" ":devicePrice.maxPrice)+($scope.priceUnit ==0?"元/月":$scope.priceUnit==1?"元/天":"元/小时");
					$scope.arraynameArray[$scope.arraynameArray.length] = devicePrice;
				}
			}
		}
		}; */
...
<body ng-app="searchApp" ng-controller="searchController" >
	...
	<div class="zbgg_t bgcfe">
		    ...
		    <ul class="ihha_sstj">
		      <li ng-repeat="a in arraynameArray">
...
white-space: nowrap;/*强制不换行*/" title="{{a.name}}" ng-cloak> 
		      ...
		      <a href="#" ng-click="deletcondition(a.type)"><img style="vertical-align:baseline;" src="../../../media/images/ssas.png"/></a></li>
		    </ul>
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_tt">
        <tr class="osf">
          ...
          <input type="checkbox" ng-model="reason2.rec" ng-click="CDClick();" />是否包含下级
          </div>
          <div style="position: absolute;top:40px;left: 0px;display: none;width:230px;border: 1px solid #bebebe;background: white;text-align: left;" id="workitem">
          <ul id="workitemulli">
...
text-overflow: ellipsis;/*文字隐藏后添加省略号*/
white-space: nowrap;/*强制不换行*/" title="{{d.name}}">{{d.name}}</div></li>
          </ul>
...
<div style="clear: both;"></div>
 ...
 <td  colspan="2" class="jjk" style="position:relative; z-index:101;">
      <input ng-model="abcInputValue" ng-change="choiceInputChange('city',abcInputValue);" ng-click="cityAClick('A');"  type="text" class="inpt_a inpt_o span230"/>
            <div class="date_css" id="riqi1" style="display:none;" onmouseover="riqi1.style.display=''">
              <p class="date_top"><font>支持中文/拼音/简拼输入</font> <a href="javascript:void(0)" onclick="riqi1.style.display='none'">×</a></p>
              <ul class="date_title">
              <!--   <li>热门</li> -->
                <li class="date_xz" ng-click="cityAClick('A');" id="ali">ABCD</li>
					     <li  ng-click="cityEClick('E');" id="eli">EFGH</li>
					      <li  ng-click="cityJClick('J');" id="jli">JKLM</li>
					       <li  ng-click="cityNClick('N');" id="nli">NOPQRS</li>
					         <li   ng-click="cityTClick('T');" id="tli">TUVWX</li>
					           <li  ng-click="cityYClick('Y');" id="yli">YZ</li>
					       <div style="clear: both;"></div>
              </ul>
              <div class="date_include">
                <div id="citya">
                 <p>A</p>
                 <ul>
                 <li ng-repeat="v in cityValueListA" ng-click="valuClick(v.name);" title="{{v.name}}"><a href="javascript:void(0);">{{v.name | limitTo:5}}</a></li>
                 </ul>
                 <div class="clear"></div>
               
                 <p>B</p>
                 <ul>
               <li ng-repeat="v in cityValueListB" ng-click="valuClick(v.name);"  title="{{v.name}}"><a href="javascript:void(0);">{{v.name | limitTo:5}}</a></li>
                </ul>
                 <div class="clear"></div>
               
                 <p>C</p>
                 <ul>
                 <li ng-repeat="v in cityValueListC" ng-click="valuClick(v.name);"  title="{{v.name}}"><a href="javascript:void(0);">{{v.name | limitTo:5}}</a></li>
                 </ul>
                 <div class="clear"></div>
               
                 <p>D</p>
                 <ul>
               	<li ng-repeat="v in cityValueListD" ng-click="valuClick(v.name);"  title="{{v.name}}"><a href="javascript:void(0);">{{v.name | limitTo:5}}</a></li>
               	</ul>
                 <div class="clear"></div>
                </div>
               <div id="citye">
                 <p>E</p>
                 <ul>
                 <li ng-repeat="v in cityValueListE" ng-click="valuClick(v.name);"  title="{{v.name}}"><a href="javascript:void(0);">{{v.name | limitTo:5}}</a></li>
                 </ul>
                 <div class="clear"></div>
               
                 <p>F</p>
                 <ul>
               <li ng-repeat="v in cityValueListF" ng-click="valuClick(v.name);"  title="{{v.name}}"><a href="javascript:void(0);">{{v.name | limitTo:5}}</a></li> </ul>
                 <div class="clear"></div>
              
                 <p>G</p>
                 <ul>
                <li ng-repeat="v in cityValueListG" ng-click="valuClick(v.name);"  title="{{v.name}}"><a href="javascript:void(0);">{{v.name | limitTo:5}}</a></li>
                 </ul>
                 <div class="clear"></div>
                
                 <p>H</p>
                 <ul>
                 <li ng-repeat="v in cityValueListH" ng-click="valuClick(v.name);"  title="{{v.name}}"><a href="javascript:void(0);">{{v.name | limitTo:5}}</a></li>
                 </ul>
                 <div class="clear"></div>
                </div>
                             <div id="cityj">      
                 <p>J</p>
                 <ul>
               <li ng-repeat="v in cityValueListJ" ng-click="valuClick(v.name);"  title="{{v.name}}"><a href="javascript:void(0);">{{v.name | limitTo:5}}</a></li>
                </ul>
                 <div class="clear"></div>
                
                 <p>K</p>
                 <ul>
               <li ng-repeat="v in cityValueListK" ng-click="valuClick(v.name);"  title="{{v.name}}"><a href="javascript:void(0);">{{v.name | limitTo:5}}</a></li>
                 </ul>
                 <div class="clear"></div>
               
                 <p>L</p>
                 <ul>
                <li ng-repeat="v in cityValueListL" ng-click="valuClick(v.name);"  title="{{v.name}}"><a href="javascript:void(0);">{{v.name | limitTo:5}}</a></li>
                 </ul>
                 <div class="clear"></div>
               
                 <p>M</p>
                 <ul>
               <li ng-repeat="v in cityValueListM" ng-click="valuClick(v.name);"  title="{{v.name}}"><a href="javascript:void(0);">{{v.name | limitTo:5}}</a></li>
                 </ul>
                 <div class="clear"></div>
                </div>
                             <div id="cityn">
                 <p>N</p>
                 <ul>
             <li ng-repeat="v in cityValueListN" ng-click="valuClick(v.name);"  title="{{v.name}}"><a href="javascript:void(0);">{{v.name | limitTo:5}}</a></li>
                 </ul>
                 <div class="clear"></div>

                 <p>P</p>
                 <ul>
            <li ng-repeat="v in cityValueListP" ng-click="valuClick(v.name);"  title="{{v.name}}"><a href="javascript:void(0);">{{v.name | limitTo:5}}</a></li>
                 </ul>
                 <div class="clear"></div>
   
                 <p>Q</p>
                 <ul>
          <li ng-repeat="v in cityValueListQ" ng-click="valuClick(v.name);"  title="{{v.name}}"><a href="javascript:void(0);">{{v.name | limitTo:5}}</a></li>
                 </ul>
                 <div class="clear"></div>

                 <p>R</p>
                 <ul>
          <li ng-repeat="v in cityValueListR" ng-click="valuClick(v.name);"  title="{{v.name}}"><a href="javascript:void(0);">{{v.name | limitTo:5}}</a></li>
                 </ul>
                 <div class="clear"></div>
    
                 <p>S</p>
                 <ul>
           <li ng-repeat="v in cityValueListS" ng-click="valuClick(v.name);"  title="{{v.name}}"><a href="javascript:void(0);">{{v.name | limitTo:5}}</a></li>
                 </ul>
                 <div class="clear"></div>
                </div>
                             <div id="cityt">
                 <p>T</p>
                 <ul>
        <li ng-repeat="v in cityValueListT" ng-click="valuClick(v.name);"  title="{{v.name}}"><a href="javascript:void(0);">{{v.name | limitTo:5}}</a></li>
                 </ul>
                 <div class="clear"></div>

                 <p>W</p>
                 <ul>
          <li ng-repeat="v in cityValueListW" ng-click="valuClick(v.name);"  title="{{v.name}}"><a href="javascript:void(0);">{{v.name | limitTo:5}}</a></li>
                 </ul>
                 <div class="clear"></div>

                 <p>X</p>
                 <ul>
         <li ng-repeat="v in cityValueListX" ng-click="valuClick(v.name);"  title="{{v.name}}"><a href="javascript:void(0);">{{v.name | limitTo:5}}</a></li>
                 </ul>
                 <div class="clear"></div>
                </div>
                             <div id="cityy">
                 <p>Y</p>
                 <ul>
        <li ng-repeat="v in cityValueListY" ng-click="valuClick(v.name);"  title="{{v.name}}"><a href="javascript:void(0);">{{v.name | limitTo:5}}</a></li>
                 </ul>
                 <div class="clear"></div>

                 <p>Z</p>
                 <ul>
         <li ng-repeat="v in cityValueListZ" ng-click="valuClick(v.name);"  title="{{v.name}}"><a href="javascript:void(0);">{{v.name | limitTo:5}}</a></li>
                 </ul>
                 <div class="clear"></div>
                </div>
              </div>
...
<a class="dfsfg" href="javascript:void(0);" id="a3" style="background: #bebebe;" ng-show="showa3">更多  ∨</a>
        ...
        <tr class="osf">
          ...
          <td valign="top"><div align="center"><a class=" col_lan" href="javascript:void(0)"  ng-click="deletcondition('device')">全部</a></div></td>
                    ...
                    <input   type="text"
          ng-click="deviceNameAClick('A','deviceNameAID');" 
          ng-model="deviceNameInputValue" 
          ng-change="choiceInputChange('device',deviceNameInputValue);"
           class="inpt_a inpt_o span230"/>
          <div class="date_css" id="riqi2" style="display:none;" onmouseover="riqi2.style.display=''">
              <p class="date_top"><font>支持中文/拼音/简拼输入</font> <a href="javascript:void(0)" onclick="riqi2.style.display='none'">×</a></p>
              <ul class="date_title">
               <!--  <li>热门</li> -->
					    <li ng-click="deviceNameAClick('A','deviceNameAID');" id="ali1">ABCD</li>
					     <li   ng-click="deviceNameEClick('E','deviceNameEID');" id="eli1">EFGH</li>
					      <li  ng-click="deviceNameJClick('J','deviceNameJID');" id="jli1">JKLM</li>
					       <li  ng-click="deviceNameNClick('N','deviceNameNID');" id="nli1">NOPQRS</li>
					         <li   ng-click="deviceNameTClick('T','deviceNameTID');" id="tli1">TUVWX</li>
					           <li ng-click="deviceNameYClick('Y','deviceNameYID');" id="yli1">YZ</li>
					       <div style="clear: both;"></div>
              </ul>
              <div class="date_include">
                <div id="devicea">
                 <p>A</p>
                 <ul>
             	<li ng-repeat="d in deviceNameValueListA" ng-click="deviceNameValueClick(d);" title="{{d.equipmentName}}"><a href="javascript:void(0);">{{d.equipmentName | limitTo:5}}</a></li>
					         </ul>
                 <div class="clear"></div>
                
                 <p>B</p>
                 <ul>
           	<li ng-repeat="d in deviceNameValueListB" ng-click="deviceNameValueClick(d);" title="{{d.equipmentName}}"><a href="javascript:void(0);">{{d.equipmentName | limitTo:5}}</a></li>
					         </ul>
                 <div class="clear"></div>
                
                 <p>C</p>
                 <ul>
                	<li ng-repeat="d in deviceNameValueListC" ng-click="deviceNameValueClick(d);" title="{{d.equipmentName}}"><a href="javascript:void(0);">{{d.equipmentName | limitTo:5}}</a></li>
					      </ul>
                 <div class="clear"></div>
               
                 <p>D</p>
                 <ul>
              	<li ng-repeat="d in deviceNameValueListD" ng-click="deviceNameValueClick(d);" title="{{d.equipmentName}}"><a href="javascript:void(0);">{{d.equipmentName | limitTo:5}}</a></li>
					      </ul>
                 <div class="clear"></div>
                </div>
                             <div  id="devicee">
                 <p>E</p>
                 <ul>
              	<li ng-repeat="d in deviceNameValueListE" ng-click="deviceNameValueClick(d);" title="{{d.equipmentName}}"><a href="javascript:void(0);">{{d.equipmentName | limitTo:5}}</a></li>
					        </ul>
                 <div class="clear"></div>
                 
                 <p>F</p>
                 <ul>
         	<li ng-repeat="d in deviceNameValueListF" ng-click="deviceNameValueClick(d);" title="{{d.equipmentName}}"><a href="javascript:void(0);">{{d.equipmentName | limitTo:5}}</a></li>
					           </ul>
                 <div class="clear"></div>
                
                 <p>G</p>
                 <ul>
      	<li ng-repeat="d in deviceNameValueListG" ng-click="deviceNameValueClick(d);" title="{{d.equipmentName}}"><a href="javascript:void(0);">{{d.equipmentName | limitTo:5}}</a></li>
					               </ul>
                 <div class="clear"></div>
                
                 <p>H</p>
                 <ul>
           	<li ng-repeat="d in deviceNameValueListH" ng-click="deviceNameValueClick(d);" title="{{d.equipmentName}}"><a href="javascript:void(0);">{{d.equipmentName | limitTo:5}}</a></li>
					          </ul>
                 <div class="clear"></div>
                </div>            <div  id="devicej">
                 <p>J</p>
                 <ul>
        	<li ng-repeat="d in deviceNameValueListJ" ng-click="deviceNameValueClick(d);" title="{{d.equipmentName}}"><a href="javascript:void(0);">{{d.equipmentName | limitTo:5}}</a></li>
					             </ul>
                 <div class="clear"></div>
                
                 <p>K</p>
                 <ul>
         	<li ng-repeat="d in deviceNameValueListK" ng-click="deviceNameValueClick(d);" title="{{d.equipmentName}}"><a href="javascript:void(0);">{{d.equipmentName | limitTo:5}}</a></li>
					            </ul>
                 <div class="clear"></div>
                
                 <p>L</p>
                 <ul>
          	<li ng-repeat="d in deviceNameValueListL" ng-click="deviceNameValueClick(d);" title="{{d.equipmentName}}"><a href="javascript:void(0);">{{d.equipmentName | limitTo:5}}</a></li>
					           </ul>
                 <div class="clear"></div>
               
                 <p>M</p>
                 <ul>
          	<li ng-repeat="d in deviceNameValueListM" ng-click="deviceNameValueClick(d);" title="{{d.equipmentName}}"><a href="javascript:void(0);">{{d.equipmentName | limitTo:5}}</a></li>
					           </ul>
                 <div class="clear"></div>
                </div>            <div  id="devicen">
                 <p>N</p>
                 <ul>
           	<li ng-repeat="d in deviceNameValueListN" ng-click="deviceNameValueClick(d);" title="{{d.equipmentName}}"><a href="javascript:void(0);">{{d.equipmentName | limitTo:5}}</a></li>
					           </ul>
                 <div class="clear"></div>
                 
                 <p>P</p>
                 <ul>
           	<li ng-repeat="d in deviceNameValueListP" ng-click="deviceNameValueClick(d);" title="{{d.equipmentName}}"><a href="javascript:void(0);">{{d.equipmentName | limitTo:5}}</a></li>
					          </ul>
                 <div class="clear"></div>
                 
                 <p>Q</p>
                 <ul>
          	<li ng-repeat="d in deviceNameValueListQ" ng-click="deviceNameValueClick(d);" title="{{d.equipmentName}}"><a href="javascript:void(0);">{{d.equipmentName | limitTo:5}}</a></li>
					           </ul>
                 <div class="clear"></div>
                 
                 <p>R</p>
                 <ul>
         	<li ng-repeat="d in deviceNameValueListR" ng-click="deviceNameValueClick(d);" title="{{d.equipmentName}}"><a href="javascript:void(0);">{{d.equipmentName | limitTo:5}}</a></li>
					           </ul>
                 <div class="clear"></div>
                 
                 <p>S</p>
                 <ul>
            	<li ng-repeat="d in deviceNameValueListS" ng-click="deviceNameValueClick(d);" title="{{d.equipmentName}}"><a href="javascript:void(0);">{{d.equipmentName | limitTo:5}}</a></li>
					         </ul>
                 <div class="clear"></div>
                </div>
                 ...
                 <p>T</p>
                 <ul>
            	<li ng-repeat="d in deviceNameValueListT" ng-click="deviceNameValueClick(d);" title="{{d.equipmentName}}"><a href="javascript:void(0);">{{d.equipmentName | limitTo:5}}</a></li>
					         </ul>
                 <div class="clear"></div>
                 
                 <p>W</p>
                 <ul>
          	<li ng-repeat="d in deviceNameValueListW" ng-click="deviceNameValueClick(d);" title="{{d.equipmentName}}"><a href="javascript:void(0);">{{d.equipmentName | limitTo:5}}</a></li>
					          </ul>
                 <div class="clear"></div>
               
                 <p>X</p>
                 <ul>
        	<li ng-repeat="d in deviceNameValueListX" ng-click="deviceNameValueClick(d);" title="{{d.equipmentName}}"><a href="javascript:void(0);">{{d.equipmentName | limitTo:5}}</a></li>
					             </ul>
                 <div class="clear"></div>
                </div>
                        <div  id="devicey">
                 <p>Y</p>
                 <ul>
            	<li ng-repeat="d in deviceNameValueListY" ng-click="deviceNameValueClick(d);" title="{{d.equipmentName}}"><a href="javascript:void(0);">{{d.equipmentName | limitTo:5}}</a></li>
					         </ul>
                 <div class="clear"></div>
       
                 <p>Z</p>
                 <ul>
         	<li ng-repeat="d in deviceNameValueListZ" ng-click="deviceNameValueClick(d);" title="{{d.equipmentName}}"><a href="javascript:void(0);">{{d.equipmentName | limitTo:5}}</a></li>
					           </ul>
                 <div class="clear"></div>
                </div>
        ...
        <tr class="osf">
          ...
          <td colspan="2"> 
          <input ng-model="minPrice" id="minPrice"   type="text" class="inpt_a inpt_o span230" style="width: 60px;" ng-blur="checkPrice(true,minPrice);" maxlength="10"/>
          <div style="float: left;margin-right: 10px;border:1px solid #bebebe;width:20px;margin-top:15px;"></div>
          <input  ng-model="maxPrice" id="maxPrice" maxlength="10" type="text" class="inpt_a inpt_o span230" style="width: 60px;"  ng-blur="checkPrice(false,maxPrice)"/> <div style="float: left;margin-right: 10px;">万元</div>
            <input  type="submit" class="moneysub" name="button" id="button" value="确定" style="margin-top: 2px;" ng-click="inputPrice('万元');"/><div class="left mar_l20"></div>
             <div style="clear: both;"></div> 
            </td>
   ...
   <div style=" margin-top:50px;"  ng-init="queryResourec();">
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_hj">
	        ...
	        <!-- |orderBy:'releaseDate':true -->
	         <tr ng-repeat="t in resultList">
				...
				<!-- <td  ng-cloak title="{{t.frontRange}}">{{t.frontRangeA}}</td> -->
				
						<td style="text-align: center;" >
				<div style="margin:0 auto;width: 130px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;color: black;" title="{{t.frontRangeA}}" ng-cloak>{{t.frontRangeA}}</div>
				</td>
					<td style="text-align: center;">
				...
				<!-- <td ng-cloak title="{{t.equName}} {{t.frontModel}}">{{t.equName}}<br/>{{t.frontModelA}}</td> -->
				<td  style="text-align: center;">
				...
				<td ng-cloak title="{{t.purchaseDate | limitTo:10}}">{{t.purchaseDate | limitTo:10}}</td>
						<td style="text-align: center;" >
				<div style="margin:0 auto;width: 130px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;color: black;" title="{{t.orgName}}" ng-cloak>{{t.orgName}}</div>
				</td>
				<!-- <td  ng-cloak title="{{t.orgName}}">{{t.orgName}}</td> -->
				
				<td ng-cloak >{{ct.codeTranslate(t.equState,"EQU_STATE")}}</td>
				<td ng-cloak>{{ct.codeTranslate(t.busState,"WORK_STATE")}}</td>
				
						<td style="text-align: center;" >
				<div style="margin:0 auto;width: 130px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;color: black;" title="{{t.equAtOrgName}}" ng-cloak>{{t.equAtOrgName}}</div>
				</td>
				<!-- <td ng-cloak>{{t.equAtOrgName}}</td> -->
