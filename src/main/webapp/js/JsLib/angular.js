...
/**
 * (modify based native angularJs : confirm ie version)
...
function copy(source, destination, stackSource, stackDest) {
  ...
  if (!destination) {
    ...
    if (source) {
      ...
      var emptyObject = Object.create(source.constructor.prototype);
...
//(modify based native angularJs : judge and create XMLHttpRequest in different way)
function createXhr(method) {
  // IE8 doesn't support PATCH method, but the ActiveX object does
  // IE6 just can use ActiveXObject
  /* global ActiveXObject */
  return (msie<=6||(msie <= 8 && lowercase(method) === 'patch'))
...
//(modify based native angularJs : send ajax request reference)
function createHttpBackend($browser, createXhr, $browserDefer, callbacks, rawDocument) {
  ...
  return function(method, url, post, callback, headers, timeout, withCredentials, responseType) {
    ...
    if (lowercase(method) == 'jsonp') {
      ...
      } catch (e) {//TODO here catch for ie6, because in ie6 xhr.onerror is undefined,will throw exception. maybe there's a better way.
      }
      try {
    	  xhr.onabort = requestError;
      } catch (e) {//TODO here catch for ie6, because in ie6 xhr.onabort is undefined,will throw exception. maybe there's a better way.
      }
      if (withCredentials) {
...
function $SceProvider() {
  ...
  this.$get = ['$parse', '$sceDelegate', function(
                $parse,   $sceDelegate) {
    //(modify based native angularJs : allow quirks mode run over ie6. but this is insecure. maybe there's a better way.)
