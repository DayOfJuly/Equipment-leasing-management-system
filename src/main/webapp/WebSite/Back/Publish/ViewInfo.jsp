...
<head>
	...
	<script type="text/javascript">
        ...
        app.controller('viewinfoController', function ($scope,$timeout,$interval,rentSvc,personSvc,SaleSvc,PicSvc,published,regionSvc,busDealInfo,busPublishInfo,PicUrl,SYS_CODE_CON,sysCodeTranslateFactory,IssuSvc) {
             ...
             $scope.querySimilar = function(){
            	 ...
            	 if ($scope.requestParms.infoType == 1) {
            		 function qSuc(rec) {
	                	 ...
	                	 IssuSvc.post({Action:"Rent"},{dataType:1,equName:$scope.equName,pageNo:0,pageSize:10},succ,err);
	            	 }
	            	 function qEr(rec) {};
	            	published.unifydo({Action:"Rent"},{parm: $scope.requestParms.id},qSuc,qEr);
	            	 ...
	            	 function qEr(rec) {};
	                published.unifydo({Action:"Sale"},{parm: $scope.requestParms.id},qSuc,qEr);
            	 } 
            };
            $scope.querySimilar();
         ...
         <div class="beft_top_right">
           ...
           <p>设备归属：<span class="txt14" style="color:black" title="{{viewList.enterpriseName}}"> {{viewList.enterpriseNameA}}</span></p>
			...
			<div ng-show="requestParms.infoType=='1'">
                                <p >设备报价：{{viewList.price}}元/{{ct.codeTranslate(viewList.priceType,"UNIT_LEASETIME")}}</p>
	                          	<p >最短租期：   <span class="col_chen txt14">{{viewList.shortestLease}}{{ct.codeTranslate(viewList.priceType,"UNIT_LEASETIME")}}</span> </p> 
            </div>
