app.controller('OutEmployeeController', function($scope,$timeout,$upload,CheckEmployeeTrueOrFalseOutSvc,personSvc,roleSvc,entSvc,personMsgSvc,downloadSvc,authoritySvc,SYS_CODE_CON,sysCodeTranslateFactory,regionSvc,personOutEmployeeSvc,published,proSvc,PicUrl) {
...
/*	//假设当前登录的用户所在的企业ID是5
	...
	$scope.queryEmployeeList = function(PageNo){
		...
		function qErr(){}
		personOutEmployeeSvc.queryPersonList({
			parTypeId:3,
		...
		if(userFunList){
			for (var i = 0; i < $scope.authorityListTemp.length; i++) {
				for(var j = 0; j < userFunList.length; j++){
					...
					if($scope.authorityListTemp[i].functionId!=33)
					{
						if(i%2==0)
						{
...
//					}
	...
	$scope.getCheckRoles = function(objList_){
		...
		for(var j = 0; j < $scope[objList_].length; j++){
			if($scope[objList_][j] && $scope[objList_][j].functionId!=33){
...
//	};
	...
	$scope.getEmployee = function(){
		if(!$scope.partyId){
			...
			personOutEmployeeSvc.queryPerson({id:$scope.partyId},
			function qSucc(rec){
...
//				}
		    ...
		    $scope.employeeEntityAdd.mail = '';
		    $scope.employeeEntityAdd.phoneNo = '';
		    $scope.employeeEntityAdd.orgName = SYS_USER_INFO.orgName;
...
//			}
		...
		/*	$scope.getAuthorityList();
			...
			personOutEmployeeSvc.addPersonDetail($scope.employeeEntityAdd,function(rec){
	...
	$scope.updateEmployeeEntity = function(obj){
		if(obj.$valid){
			...
			personOutEmployeeSvc.updatePerson({id:$scope.employeeEntityAdd.partyId},$scope.employeeEntityAdd,function(rec){
				$scope.showFlag='';
				$scope.queryTemPartyList(1);
	...
	$scope.delEmployee = function(varl){
		if(!$scope.partyId){
			...
			personOutEmployeeSvc.queryPerson({id:$scope.partyId},function(rec){
				if (rec.state == 0) {
					$scope.stateList = rec;
	...
	$scope.stateSubmit = function(){
			...
			personOutEmployeeSvc.oPerson({id:$scope.partyId,state:($scope.employeeEntityAdd.state == 0?1:0)},qSucc,qErr);
	};
	...
	*/
	...
	$scope.deletePic=function(parm,obj){
		...
		personOutEmployeeSvc.deletePersonPic({urlPath:"pic",id:$scope.employeeEntityAdd.partyId,upLoadId:obj.uploadId},succ,err);
	};
