...
app.controller('searchController',function($scope,entSvc,busDealInfo,$timeout,proSvc,busPublishHist,busPublishInfo,rentSvc,SaleSvc,SYS_CODE_CON,sysCodeTranslateFactory,DemandRentSvc,DemandSaleSvc,published,busAuditSvc){
	...
	*/
	...
	$scope.userInfo.orgParTypeId = "${sessionScope.userInfo.orgParTypeId}";
	...
	$scope.addPublish = function(rec){
		function qSucc(rec){
    		...
    		if(rec.publishInfo!==null){
    			if($scope.addPublishList.busState==null){
        			if($scope.userInfo.orgParTypeId==8){
    	...
    	function qErr(){}
    	busPublishHist.unifydo({ urlPath: rec.dataId},qSucc,qErr);
	...
	$scope.openEmployerModel = function(){
		if($scope.employers.length==0){//	首次打开
			...
			if(2==orgLv){
				...
				$scope.check = true;
				if($scope.userInfo.orgLevel == 1&&SYS_USER_INFO.orgLevel==2){
					$scope.queryEmployer.nonCurrOrgId = SYS_USER_INFO.orgId;
				}else{
					$scope.queryEmployer.nonCurrOrgId = $scope.parentOrg.parentOrgId;
				}
        ...
        <tr >
       	 ...
       	 <tr ng-repeat="p in publishList">
						...
						<!-- 	<a href="" style="text-decoration: none;margin-left:5px;color: blue"  ng-show="p.dataState!=5"  ng-click="delPublish(p.id);">作废</a> -->
							...
							<a ng-if="p.dataType==1 && p.dataState==3"  style="text-decoration: none;margin-left:5px;color: blue" ng-click="addPublish(p);">发布结果登记</a>
							<a ng-if="p.dataType==2 && p.dataState==3" style="text-decoration: none;margin-left:5px;color: blue" ng-click="addPublish(p);">发布结果登记</a>
