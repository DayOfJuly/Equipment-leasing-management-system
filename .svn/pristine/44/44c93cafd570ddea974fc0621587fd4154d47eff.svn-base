...
<script type="text/javascript">
	var app = angular.module('DemandRentApp', [ 'ngResource', 'unifyModule',
			'myPagination' ]);
	app.controller('DemandRentController', function($scope,DemandRentSvc) {
		 ...
		 * 查询test数据
		 */
		$scope.queryDemandRentData = function() {
		...
		*/
		...
		$scope.add = function() {
			...
			DemandRentSvc.put({data : $scope.formParms}, aSucc, aErr);
			function aSucc(rec) {
				$('#rentModalId').modal('hide');
				$.messager.popup("求租信息"+rec.msg);
			}
			function aErr(rec) {
		...
		$scope.openView = function(id) {
			window.location.href = "RentView.jsp?id=" + (id-1);
		};
		/*
		 * 修改
		 */
		$scope.upd = function() {
			...
			DemandRentSvc.post({parm : id},$scope.formParms, sSucc, sErr);
			function sSucc(rec) {
				$('#rentModalId').modal('hide');
		...
		$scope.del = function(obj) {
			$.messager.confirm("提示", "是否删除？", function() {
				var id = obj.id;
				DemandRentSvc.del({parm:id}, dSucc, dErr);
				function dSucc(rec) {
					$.messager.popup(rec.msg);
				}
				function dErr(rec) {
...
<body ng-app="DemandRentApp" ng-controller="DemandRentController">
	<ol class="breadcrumb">
	...
	<form action="" style="width: 95%">
		...
		<div ng-include src="'/WebFront/WebSite/DemandRent/DemandRentAM.jsp'"></div>
	</form>
</body>
</html>
